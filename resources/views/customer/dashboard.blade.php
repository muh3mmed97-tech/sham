...
<div style="display: flex; justify-content: center; gap: 15px; padding: 10px 15px 25px 15px; flex-wrap: wrap;">
    <a href="{{ route('customer.dashboard') }}" style="text-decoration: none; color: #005a9c; font-weight: bold; padding: 10px 20px; border-radius: 20px; background: #fff; border: 1px solid #ddd; transition: 0.3s;">الكل</a>
    @foreach(\App\Models\Category::all() as $cat)
        <a href="{{ route('customer.dashboard', ['category' => $cat->id]) }}" style="text-decoration: none; color: #005a9c; font-weight: bold; padding: 10px 20px; border-radius: 20px; background: #fff; border: 1px solid #ddd; transition: 0.3s;">{{ $cat->name }}</a>
    @endforeach
</div>
                ...
                <div style="margin-top: auto; display: flex; gap: 10px;">
                    ...
                    <form action="{{ route('wishlist.toggle', $product->id) }}" method="POST" style="flex: 0 0 50px;">
                        ...
                        <button type="submit" style="padding: 10px; border: none; border-radius: 8px; cursor: pointer; 
                            ...
                            color: {{ auth()->user()->wishlist->contains('product_id', $product->id) ? 'white' : 'black' }};">❤️</button>
                    </form>
                </div>
            </div>
        @empty
